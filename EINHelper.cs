	...
	public partial class EINHelper : Form {
		...
		public EINHelper() {
			try {
				...
				string mapCsv = null;
				using (StreamReader sr = new StreamReader(asm.GetManifestResourceStream("EndIsNigh.map.csv"))) {
					mapCsv = sr.ReadToEnd();
				}
					...
					for (int j = 0; j < map.Columns.Count; j++) {
						...
						if (!string.IsNullOrEmpty(mapID)) {
							if (mapID != "..") {
								...
								cells[i, j] = new MiniMapCell(mapID, entry?.Name, j, i, CellType.Normal, (entry?.Color).GetValueOrDefault(Color.White));
							} else {
